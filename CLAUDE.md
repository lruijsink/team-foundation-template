...
- `./lint.sh <path>...` - lint one or more files/dirs, recursively (check only, no fixes). Files/dirs are required args;
  running with none prints usage and exits 1.
- `./format.sh <path>...` - auto-format one or more files/dirs, recursively. Fixes what it can, reports what it can't
  (e.g. ktlint can't auto-fix wildcard imports).
- `./lint-ci.sh` - lints every tracked file in the repo (`git ls-files`). This is the source of truth for whether the
  repo is clean; run this to reproduce what CI checks.
...
To lint/format a single file while iterating: `./lint.sh path/to/File.kt` or `./format.sh path/to/File.kt`.
...
- `git-hooks/pre-commit` - lints staged files, blocks the commit on failure. Because a file can be partially staged, it
  ...
  directory search, and Prettier hardcodes `.git` as an ignored path (any file under `.git/` is silently skipped).
...
A `PostToolUse` hook (`.claude/settings.json`, matcher `Edit|Write`) runs `format.sh` on any file Claude edits or
